...
use App\Models\Betsuper;
use App\Models\Betsuperanalise;
use Carbon\Carbon;
use Illuminate\Http\Request;

class BetsuperController extends Controller
{
    protected $entity, $betanalise;

    public function __construct (Betsuper $bet, Betsuperanalise $beta)
    {
        $this->entity = $bet;
    ...
    public function store(Request $request)
    {
        ...
        foreach ($data as $item) {
            for ($i=1; $i < 21; $i++) { 
                ...
                if (sizeof($novo) == 5 && strcmp($novo[4], '?') != 0 && $novo[0] == "SUPER") {
                    $dt = explode("/", $novo[1]);
                    $data = $dt[2]."-".$dt[1]."-".$dt[0];
