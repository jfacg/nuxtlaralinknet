...
class BetcopaController extends Controller
{
    protected $entity, $betanalise;

    public function __construct (Betcopa $bet, Betcopaanalise $beta)
    {
        $this->entity = $bet;
    ...
    public function store(Request $request)
    {
        ...
        foreach ($data as $item) {
            for ($i=1; $i < 21; $i++) { 
                ...
                if (sizeof($novo) == 5 && strcmp($novo[4], '?') != 0 && $novo[0] == "COPA") {
                    $dt = explode("/", $novo[1]);
                    $data = $dt[2]."-".$dt[1]."-".$dt[0];
        ...
        foreach ($bets as $key => $jogo) {
            ...
            if (!is_null($jogoe) && !is_null($jogod)) {
                ...
                if (!is_null($jogocentro) && !is_null($jogoesquerda) && !is_null($jogodireita)) {
                    ...
                    if ($this->entity->verificarPlacar($jogoesquerda->placar, 'over')) {
                        $green = $green."1";
                    }
                    
                    if ($this->entity->verificarPlacar($jogocentro->placar, 'over')) {
                        $green = $green."2";
                    }
                    
                    if ($this->entity->verificarPlacar($jogodireita->placar, 'over')) {
                        $green = $green."3";
                    }
